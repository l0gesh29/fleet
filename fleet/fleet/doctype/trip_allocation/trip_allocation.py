...
class TripAllocation(Document):
	def before_save(self):
		...
		if self.saved and self.vehicle_type=="Own Vehicle":
			...
			#	if res:
			#		self.lorry_hire= res
					self.total_lorry_hire = self.lorry_hire+self.hamali+self.toll
			#	else:
			#		frappe.throw("No price for "+self.loading_point_c+" to "+self.destination_c+" in  LH Master")
			#else:
			#	frappe.throw("No record in LH Master for  "+self.consignor_c)
		else:
			self.saved=1
	...
	def on_submit(self):
		for row in self.dr_table_c:
			...
			if self.vehicle_type == "Market Vehicle":
				...
				doc.market_lorry_hire_c = self.market_total_lorry_hire
...
def o_amount(self,method):
	if self.references:
		if self.references[0].reference_doctype == "Final Trip Sheet":
			doc=frappe.get_doc("Final Trip Sheet",self.references[0].reference_name)
#			doc.mode_of_payment_c = self.mode_of_payment
	#	doc.ref_date_c = self.posting_date
			...
			if doc.outstanding_amount:
				...
				doc.advance_amount += self.paid_amount
	#		else:
